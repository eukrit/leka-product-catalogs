...
import { VendorProductCard } from "@/components/vendors/vendor-product-card"
...
function CatalogContent({ brand }: { brand: BrandConfig }) {
  const vendorTheme = getVendorTheme(brand.slug)
  ...
  return (
    <>
      {/* Vendor hero (replaces generic stats header for themed brands) */}
      {vendorTheme && (
        <VendorHero brand={brand} theme={vendorTheme} totalCount={totalCount} />
      )}

      <main className="max-w-7xl mx-auto px-6 py-8">
        {/* Generic stats header for non-themed brands */}
        {!vendorTheme && (
